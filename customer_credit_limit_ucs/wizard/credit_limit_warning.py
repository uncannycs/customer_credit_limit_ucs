...
class CreditLimitWarningWizard(models.TransientModel):
    ...
    credit_limit = fields.Monetary(string="Credit Limit", compute='_compute_details', currency_field='currency_id')
    total_receivables = fields.Monetary(string="Total Receivables", compute='_compute_details', currency_field='currency_id')
    pending_orders_amount = fields.Monetary(string="Pending Orders Amount", compute='_compute_details', currency_field='currency_id')
    current_order_amount = fields.Monetary(string="Current Order Amount", compute='_compute_details', currency_field='currency_id')
    total_exposure = fields.Monetary(string="Total Exposure", compute='_compute_details', currency_field='currency_id')
    exceeded_amount = fields.Monetary(string="Exceeded Amount", compute='_compute_details', currency_field='currency_id')
    ...
    @api.depends('sale_order_id', 'uninvoiced_order_ids', 'unpaid_invoice_ids')
    def _compute_details(self):
        for wizard in self:
            ...
            wizard.total_receivables = commercial_partner.credit

            pending_amount = 0.0
            company = order.company_id or self.env.company
            for po in wizard.uninvoiced_order_ids:
            ...
            wizard.total_exposure = (
                wizard.total_receivables
                + wizard.pending_orders_amount
                + wizard.current_order_amount
            )
